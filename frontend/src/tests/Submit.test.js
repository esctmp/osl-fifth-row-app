...
var validFormEPFID = 324;
var testValidFormName = "";
var spy;
beforeEach(() => {
...
describe('Fifth Row - EPF Form', () => {
    ...
    describe('Upload', () => {
        ...
        test("Valid Form Behavior 1", async () => {
            const userId = await getSampleUserId();
            ...
            testValidFormName = eventName;

        }, 30000);

        test("Valid Form Behavior 2", async () => {
            const { epf_id, exco_user_id } = await getFormWithEventName(testValidFormName);
            jest.spyOn(Router, 'useParams').mockReturnValue({ epf_id: epf_id });
            const alertMock = spy.mockImplementation(() => { })
            let dom = render(
                <UserID.Provider value={{ userId: exco_user_id, setUserId: () => { } }}>
                    <FifthRowEPFSubmit />
                </UserID.Provider>
            )
            ...
            // 38
            let form = await getFormWithEventName(testValidFormName);
